...
""",
        ...
        self.assertIn("document.body.append(liveBrief, calendarReturn, workNote, workNoteOverlay);", refreshed)
        self.assertIn("Return to the non-human timetable / 返回非人时间表", refreshed)
        self.assertIn("calendarReturnUrl.searchParams.set('date', WORK_NOTE.date)", refreshed)
        self.assertIn("function suppressNativeTitleChrome()", refreshed)
        self.assertIn("const liveBrief = createLiveBrief();", refreshed)
        self.assertIn("makeElement('aside', 'gh-live-brief is-collapsed')", refreshed)
        self.assertIn("toggle.setAttribute('aria-expanded', 'false');", refreshed)
        self.assertIn("function enableLiveBriefDragging()", refreshed)
        self.assertIn("function clampLiveBriefToViewport()", refreshed)
