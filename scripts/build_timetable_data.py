...
def load_history(path: Path) -> dict[str, dict]:
    ...
    for entry in days:
        ...
        require(1 <= len(residues) <= 6, f"{day_date} history needs 1-6 Simon × Heizhou assigned residues")
        signatures = set()
        for index, residue in enumerate(residues):
            ...
            if residue["source_kind"] in SESSION_SOURCE_KINDS:
                ...
                if residue["source_kind"] == "collaboration_session":
                    ...
                    excerpts = residue["public_excerpts"]
                    require(
                        ...
                        f"{day_date} residue {index + 1} has invalid public excerpts",
                    )
                    require(
                        residue["excerpt_provenance"]
                        == "sanitized_owner_dialogue",
                        f"{day_date} residue {index + 1} has invalid excerpt provenance",
                    )
                    require(
                        sum(excerpt.count("████") for excerpt in excerpts)
                        == residue["excerpt_redaction_count"],
                        f"{day_date} residue {index + 1} excerpt mask count mismatch",
                    )
                    for excerpt in excerpts:
                        require(
                            not URL_RE.search(excerpt)
                            and "/Users/" not in excerpt
                            and ".hermes" not in excerpt
                            and "state.db" not in excerpt
                            and not SENSITIVE_ASSIGNED_WORK_RE.search(excerpt)
                            and not PRIVATE_OPERATIONAL_CONTEXT_RE.search(excerpt)
                            and not EDUCATION_IDENTITY_RE.search(excerpt)
                            and not PROPOSAL_TITLE_CONTEXT_RE.search(excerpt),
                            f"{day_date} residue {index + 1} has an unsafe public excerpt",
                        )
            if residue["source_kind"] == "collaboration_session":
                require(
                    residue["redaction_count"] == residue["excerpt_redaction_count"],
                ...
                **(
                    {
                        ...
                        "public_excerpts": residue["public_excerpts"],
                        "excerpt_redaction_count": residue[
                            "excerpt_redaction_count"
                        ],
                        "excerpt_provenance": residue["excerpt_provenance"],
                    }
...
def climate_family_and_window(pulse: dict) -> tuple[str, str]:
    ...
    if category == "ai_daily_brief":
        return "ai_brief", "daily"
    window = "early" if start < 11 * 60 else "daytime" if start < 18 * 60 else "evening"
    return "support_checks", window
...
def climate_group_summary(pulses: list[dict]) -> tuple[str, str]:
    ...
    return (
        f"{window_count} 个精确窗口共完成 {run_count} 次服务健康检查或其他后台运行；未保留公开级别提示。",
        f"{run_count} service-health checks or other background runs completed across {window_count} exact windows; no public-level alert was retained.",
    )
