...
async function inspectViewport(browser, url, viewport) {
  ...
  const translatedCardMarkdown = card.locator(".translated-reminder-copy.markdown-content");
  const originalCardMarkdown = card.locator(".original-reminder-copy.markdown-content");
  await assertMarkdownSemantics(translatedCardMarkdown, `${viewport.label} translated card`);
  await assertMarkdownSemantics(originalCardMarkdown, `${viewport.label} original card`);
  await assertXssBoundary(page, translatedCardMarkdown, `${viewport.label} translated card`, {
    interactiveLinks: false,
  });
  await assertXssBoundary(page, originalCardMarkdown, `${viewport.label} original card`, {
    interactiveLinks: false,
  });
  ...
  assert.deepEqual(cardBars, [fixedBlock, fixedBlock], `${viewport.label}: fixed card redaction`);
  const compactState = await translatedCardMarkdown.evaluate((element) => {
    const card = element.closest(".event-reading-card");
    const summary = element.closest(".pulse-summary");
    return {
  ...
  const detailMarkdownEn = page.locator("#taskDetailEn.markdown-content");
  await assertMarkdownSemantics(detailMarkdown, `${viewport.label} original detail`);
  await assertMarkdownSemantics(detailMarkdownEn, `${viewport.label} translated detail`);
  await assertXssBoundary(page, detailMarkdown, `${viewport.label} original detail`);
  await assertXssBoundary(page, detailMarkdownEn, `${viewport.label} translated detail`);
...
try {
  ...
  fixtureDay.pulses.push({
    ...
    summary_en: markdownBody,
    excerpt_en: markdownExcerpt,
