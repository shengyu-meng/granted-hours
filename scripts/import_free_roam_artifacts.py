...
<style id="granted-hours-fold-style">
  ...
  .gh-live-brief.is-dragging .gh-live-brief-header { cursor: grabbing; }
...
(() => {
  ...
  function createLiveBrief() {
    const brief = makeElement('aside', 'gh-live-brief is-collapsed');
    brief.id = 'ghLiveBrief';
    brief.dataset.ghLiveBrief = 'bilingual';
    brief.dataset.ghDraggable = 'header';
    ...
    header.title = 'Drag the brief / 拖动说明卡片';
    ...
    toggle.setAttribute('aria-label', 'Expand bilingual brief / 展开双语简述');
  ...
  function toggleLiveBrief() {
    ...
    const body = liveBrief.querySelector('.gh-live-brief-body');
  ...
  function enableLiveBriefDragging() {
    const header = liveBrief.querySelector('.gh-live-brief-header');
    let drag = null;
    const endDrag = (event) => {
      ...
      drag = null;
      liveBrief.classList.remove('is-dragging');
      liveBrief.dataset.ghUserPositioned = 'true';
      window.requestAnimationFrame(refreshFloatingChrome);
    };
    header.addEventListener('pointerdown', (event) => {
      if (event.button !== 0) return;
      if (event.target.closest('button,a,input,textarea,select,[role="button"]')) return;
      const rect = liveBrief.getBoundingClientRect();
      drag = {
        pointerId: event.pointerId,
        offsetX: event.clientX - rect.left,
        offsetY: event.clientY - rect.top,
      };
      liveBrief.style.right = 'auto';
      liveBrief.style.bottom = 'auto';
      liveBrief.style.left = `${rect.left}px`;
      liveBrief.style.top = `${rect.top}px`;
      liveBrief.classList.add('is-dragging');
    ...
    header.addEventListener('pointermove', (event) => {
      ...
      const width = liveBrief.offsetWidth;
      const height = liveBrief.offsetHeight;
      ...
      liveBrief.style.left = `${nextLeft}px`;
      liveBrief.style.top = `${nextTop}px`;
      event.preventDefault();
      event.stopPropagation();
    });
  ...
  function clampLiveBriefToViewport() {
    if (liveBrief.dataset.ghUserPositioned !== 'true') return;
    const rect = liveBrief.getBoundingClientRect();
    const left = Math.min(
    ...
    liveBrief.style.left = `${left}px`;
    liveBrief.style.top = `${top}px`;
  }
  ...
  <main class="site">
    <section class="hero">
      ...
      <div class="actions">
        ...
        <a class="button" href="./timetable/">Enter non-human timetable / 进入非人时间表</a>
