...
class TestLiveWorkNoteOverlay(unittest.TestCase):
    def test_rendered_snippet_contains_dynamic_bilingual_note(self) -> None:
        ...
        self.assertIn("Return to the non-human timetable / 返回非人时间表", snippet)
        self.assertIn("calendarReturnUrl.searchParams.set('date', WORK_NOTE.date)", snippet)
        self.assertIn("function suppressNativeTitleChrome()", snippet)
        self.assertIn("data-gh-native-title-suppressed", snippet)
        ...
        self.assertIn("const MEDIA_VERSION = 2;", snippet)
        self.assertIn("postMediaEvent('ready', 'ready')", snippet)
        self.assertIn("retryEmbeddedMediaFromGesture", snippet)
        self.assertIn("Tap for artwork sound / 轻触开启作品声音", snippet)
        self.assertIn("postMediaEvent('state', 'buffering')", snippet)
        self.assertIn("blocked ? 'blocked' : 'error'", snippet)
        self.assertIn("window.addEventListener('load', syncEmbeddedMediaState", snippet)
        self.assertIn("if (mediaEnabled) setEmbeddedMediaState(true);", snippet)
        self.assertIn("if (!mediaUnlockRequired || event.target === mediaUnlock) return;", snippet)
    ...
    def test_all_live_pages_have_exactly_one_overlay_and_matching_payload(self) -> None:
        ...
        for page in pages:
            ...
            with self.subTest(date=date):
                ...
                self.assertIn("const MEDIA_VERSION = 2;", text)
                self.assertIn("postMediaEvent('ready', 'ready')", text)
                self.assertIn("gh-media-unlock", text)
                self.assertIn("window.setTimeout(syncEmbeddedMediaState, 250)", text)
    ...
    def test_overlay_is_closed_by_default_and_mobile_safe(self) -> None:
        ...
        self.assertIn("function alignCalendarReturn()", snippet)
        self.assertIn("function findSoundControl()", snippet)
        self.assertIn("function offsetNativeControlText()", snippet)
        self.assertIn("function restoreNativeControlOffsets()", snippet)
        self.assertIn("function nativeControlOffsetTarget(", snippet)
        self.assertIn('data-gh-control-offset="true"', snippet)
        self.assertIn('data-gh-sound-geometry="compact"', snippet)
        self.assertIn('data-gh-sound-touch-target="true"', snippet)
        self.assertIn('data-gh-sound-mobile-docked="true"', snippet)
        self.assertIn("function normalizeSoundForTouch(sound)", snippet)
        self.assertIn("function ensureWorkNoteContrast()", snippet)
        self.assertIn("function scheduleFloatingChromeRefresh()", snippet)
        self.assertIn("document.addEventListener('click', scheduleFloatingChromeRefresh)", snippet)
        self.assertIn("function isVisiblyRendered(element)", snippet)
        self.assertIn("workNote.dataset.ghControlLayout = 'stacked'", snippet)
        self.assertIn("workNote.dataset.ghControlLayout = 'alongside'", snippet)
        self.assertIn('data-gh-control-concealed="true"', snippet)
        self.assertIn("@media (max-height: 520px)", snippet)
        self.assertNotIn("function avoidOverlap(", snippet)
    ...
    def test_bilingual_live_brief_is_collapsed_by_default_and_draggable(self) -> None:
        ...
        self.assertIn("refreshFloatingChrome();\n    window.requestAnimationFrame(refreshFloatingChrome);", snippet)
        ...
        self.assertIn("isTouchLayout() && window.innerHeight <= 520", snippet)
        self.assertIn(".hint,.brief,#brief,.instructions", snippet)
        self.assertIn("const nativeBriefShell = element.matches('.brief,#brief');", snippet)
