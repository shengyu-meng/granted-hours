#!/usr/bin/env python3
"""Near-verbatim public projection for explicitly authorized reminders.

Private response sections are joined in source order and masked in memory.
Only identifying entities and structured technical secrets are changed. The
public projection contains the masked original once, an extractive card
...
def project_limited_reminder_response(
    ...
    *,
    ...
    summary = masked[:MAX_SOURCE_LENGTH]
    if len(masked) > MAX_SOURCE_LENGTH:
        summary = f"{summary[:-1]}…"
    ...
    return {
        ...
        "projection_kind": "verbatim_redacted" if redaction_count else "verbatim",
